...
import css from './OrderApp.module.css';
...
const OrderApp: FC<IProps> = ({ order, isOpen, onClick }) => {
    ...
    return (
        <>
            <StyledTableRow onClick={() => onClick()}>
                ...
                <StyledTableCell align="center">{name ? name : 'no data'}</StyledTableCell>
                <StyledTableCell align="center">{surname ? surname : 'no data'}</StyledTableCell>
                <StyledTableCell align="center">{email ? email : 'no data'}</StyledTableCell>
                <StyledTableCell align="center">{phone ? phone : 'no data'}</StyledTableCell>
                <StyledTableCell align="center">{age ? age : 'no data'}</StyledTableCell>
                <StyledTableCell align="center">{course ? course : 'no data'}</StyledTableCell>
                <StyledTableCell align="center">{course_format ? course_format : 'no data'}</StyledTableCell>
                <StyledTableCell align="center">{course_type ? course_type : 'no data'}</StyledTableCell>
                <StyledTableCell align="center">{status ? status : 'no data'}</StyledTableCell>
                <StyledTableCell align="center">{sum ? sum : 'no data'}</StyledTableCell>
                <StyledTableCell align="center">{already_paid ? already_paid : 'no data'}</StyledTableCell>
                <StyledTableCell align="center">{nameGroup ? nameGroup : 'no data'}</StyledTableCell>
                <StyledTableCell align="center">{ <DateFormat originalDate={created_at} /> }</StyledTableCell>
                <StyledTableCell align="center">{manager ? manager.name : 'no manager'}</StyledTableCell>
            </StyledTableRow>
            <StyledTableCell sx={{"&:nth-of-type(odd)": {
                ...
                <Collapse in={isOpen}>
                    <Stack direction={'row'} justifyContent={'space-between'} useFlexGap>
                        ...
                        <div className={css.right_block}>
                            {comments.length < 1 ? 'No comments.' : 'Comments:'}
                            <div
